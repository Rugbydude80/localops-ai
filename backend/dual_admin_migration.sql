-- =====================================================
-- COMPLETE DUAL ADMIN ARCHITECTURE MIGRATION
...
BEGIN
    FOR i IN 0..29 LOOP
        current_date := CURRENT_DATE - (i * INTERVAL '1 day');
        
        -- Daily revenue data
        INSERT INTO platform_analytics (metric_date, metric_name, metric_value, category) VALUES
        (current_date, 'daily_revenue', 1500 + (i * 25), 'revenue'),
        (current_date, 'new_subscriptions', 2 + (i % 3), 'subscriptions'),
        (current_date, 'cancellations', 1 + (i % 5), 'subscriptions'),
        (current_date, 'active_users', 1200 + (i * 15), 'users'),
        (current_date, 'support_tickets', 8 + (i % 4), 'support'),
        (current_date, 'security_events', 3 + (i % 2), 'security')
...
('TICK-2024-001', 1, 1, 'demo@localops.ai', 'Scheduling Issue', 'The auto-scheduling feature is not working correctly for our weekend shifts.', 'high', 'open', 'scheduling'),
('TICK-2024-002', 1, 1, 'demo@localops.ai', 'Staff Management', 'Need help setting up new staff member with proper permissions.', 'medium', 'open', 'staff_management'),
('TICK-2024-003', 1, 1, 'demo@localops.ai', 'Reporting Question', 'How can I generate custom reports for our monthly staff performance?', 'low', 'resolved', 'reporting'),
('TICK-2024-004', 1, 1, 'demo@localops.ai', 'Integration Request', 'Looking to integrate with our POS system for automatic sales data.', 'medium', 'open', 'integrations')
ON CONFLICT DO NOTHING;
